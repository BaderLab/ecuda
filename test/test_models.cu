...
void testMatrix( const ecuda::__device_contiguous_row_matrix<T,U>& matrix ) {
...
template<typename T,typename U>
__global__ void testKernel( const ecuda::__device_contiguous_row_matrix<T,U> matrix, ecuda::__device_sequence<T,U> vector ) {
	ecuda::copy( matrix.get_column(0).begin(), matrix.get_column(0).end(), vector.begin() );
}

int main( int argc, char* argv[] ) {

	ecuda::__device_sequence< int, ecuda::unique_ptr<int> > device_sequence_noncontiguous1( ecuda::unique_ptr<int>( ecuda::device_allocator<int>().allocate( 100 ) ), 100 );
	ecuda::__device_contiguous_sequence< int, ecuda::unique_ptr<int> > device_sequence_contiguous1( ecuda::unique_ptr<int>( ecuda::device_allocator<int>().allocate( 100 ) ), 100 );
	...
	ecuda::__device_sequence< double, ecuda::unique_ptr<double> > device_sequence_noncontiguous2( ecuda::unique_ptr<double>( ecuda::device_allocator<double>().allocate( 100 ) ), 100 );
	ecuda::__device_contiguous_sequence< double, ecuda::unique_ptr<double> > device_sequence_contiguous2( ecuda::unique_ptr<double>( ecuda::device_allocator<double>().allocate( 100 ) ), 100 );
...

	{
		...
		ecuda::__device_contiguous_row_matrix< int, ecuda::padded_ptr<int> > matrix( paddedPointer, 10, 20 );
		testMatrix( matrix );
		matrix.get_row(0);
		matrix.get_column(0);
	}
	...
	ecuda::__device_contiguous_row_matrix< int, ecuda::unique_ptr<int> > matrix1( ptr, 10, 20 );
	testMatrix( matrix1 );
	matrix1.get_row(0);
	matrix1.get_column(0);

	ecuda::__device_contiguous_row_matrix< int, ecuda::naked_ptr<int> > matrix2( ecuda::naked_ptr<int>(ptr.get()), 10, 20 );
	testMatrix( matrix2 );
	matrix2.get_row(0);
	matrix2.get_column(0);

	ecuda::__device_contiguous_row_matrix< int, ecuda::padded_ptr< int, ecuda::shared_ptr<int> > > matrix3( ecuda::padded_ptr< int, ecuda::shared_ptr<int> >( ecuda::shared_ptr<int>(), 100, 100, ecuda::shared_ptr<int>() ) );
	testMatrix( matrix3 );
	matrix3.get_row(0);
