...
class array_kernel_argument; // forward declaration
...
protected:
    ...
    ///
    template<class Q>
    __HOST__ __DEVICE__ array(const array<T, N, Q>& src, ecuda::true_type)
...
public:
    ...
    ///
    __HOST__ array()
      : base_type(shared_ptr<T>(device_allocator<T>().allocate(N)))
    {
        fill(value_type());
    }
    ...
    __HOST__ array(const array& src)
      : base_type(shared_ptr<T>(device_allocator<T>().allocate(src.size())))
    {
...
#ifdef ECUDA_CPP11_AVAILABLE
    template<typename U>
    __HOST__ array(std::initializer_list<U> il)
      : base_type(shared_ptr<T>(device_allocator<T>().allocate(N)))
    {
#ifdef ECUDA_CONSTEXPR_KEYWORD_ENABLED
        static_assert(il.size() <= N,
                      ECUDA_EXCEPTION_MSG("size of initializer list must not be greater than the array size"));
#else
        if (il.size() > N)
    ...
    ///
    __HOST__ ECUDA__CONSTEXPR inline size_type max_size() const ECUDA__NOEXCEPT
    {
...
// noexcept(noexcept(swap(std::declval<T&>(),std::declval<T&>())))
    ...
    ///
    __HOST__ __DEVICE__ inline bool operator==(const array& other) const
    {
    ...
    ///
    /// \param other container to compare contents with
    /// \returns true if the contents of this array are lexicographically less than or equal to the other array, false
    /// otherwise
    ///
    __HOST__ __DEVICE__ inline bool operator<=(const array& other) const { return !operator>(other); }
    ...
    ///
    /// \param other container to compare contents with
    /// \returns true if the contents of this array are lexicographically greater than or equal to the other array,
    /// false otherwise
    ///
    __HOST__ __DEVICE__ inline bool operator>=(const array& other) const { return !operator<(other); }
};
...
public:
    template<class P>
    __HOST__ array_kernel_argument(const array<T, N, P>& src)
      : base_type(src, ecuda::true_type())
    {
    }

    __HOST__ __DEVICE__ array_kernel_argument(const array_kernel_argument& src)
      : base_type(src, ecuda::true_type())
    {
    }
...
#ifdef ECUDA_CPP11_AVAILABLE
    array_kernel_argument(array_kernel_argument&& src)
      : base_type(std::move(src))
    {
    }
