...
public:
    ...
    ///
    /// In the default allocator, the block of storage is at some point deallocated using \c \::operator \c delete
    /// (either during the function call, or later).
    ///
    ...
    ///
    /// \brief Returns the maximum number of elements, each of member type value_type (an alias of allocator's template
    /// parameter)
    ///        that could potentially be allocated by a call to member allocate.
    ///
    /// A call to member allocate with the value returned by this function can still fail to allocate the requested
    /// storage.
    ///
    ...
    ///        Notice that this does not deallocate the storage for the element (see member deallocate to release
    ///        storage space).
...
public:
    ...
    ///
    /// \brief Constructs a device pitched memory allocator object from another device pitched memory allocator object
