...
class cube_kernel_argument; // forward declaration
...
public:
    ...
    typedef model::device_sequence<value_type,
                                   striding_padded_ptr<value_type, typename ecuda::add_pointer<value_type>::type>>
      row_type; //!< cube row container type
    typedef model::device_sequence<value_type,
                                   striding_padded_ptr<value_type, typename ecuda::add_pointer<value_type>::type>>
      column_type;                                                    //!< cube column container type
    typedef model::device_contiguous_sequence<value_type> depth_type; //!< cube depth container type
    typedef model::device_sequence<
      const value_type,
      striding_padded_ptr<const value_type, typename ecuda::add_pointer<const value_type>::type>>
      const_row_type; //!< cube const row container type
    typedef model::device_sequence<
      const value_type,
      striding_padded_ptr<const value_type, typename ecuda::add_pointer<const value_type>::type>>
      const_column_type;                                                          //!< cube const column container type
    ...
    typedef model::device_matrix<value_type,
                                 striding_padded_ptr<value_type, typename ecuda::add_pointer<value_type>::type>>
      slice_xy_type; //!< xy section of a cube at a fixed depth
    typedef model::device_contiguous_row_matrix<value_type, typename ecuda::add_pointer<value_type>::type>
      slice_xz_type; //!< xz section of a cube at a fixed column
    typedef model::device_contiguous_row_matrix<value_type, typename ecuda::add_pointer<value_type>::type>
      slice_yz_type; //!< yz section of a cube at a fixed row
    typedef model::device_matrix<
      const value_type,
      striding_padded_ptr<const value_type, typename ecuda::add_pointer<const value_type>::type>>
      const_slice_xy_type; //!< xy section of a cube at a fixed depth
    typedef model::device_contiguous_row_matrix<const value_type, typename ecuda::add_pointer<const value_type>::type>
      const_slice_xz_type; //!< const xz section of a cube at a fixed row
    typedef model::device_contiguous_row_matrix<const value_type, typename ecuda::add_pointer<const value_type>::type>
      const_slice_yz_type; //!< const yz section of a cube at a fixed row
...
protected:
    ...
    ///
    template<typename U>
    __HOST__ __DEVICE__ cube(const cube<T, Alloc, U>& src, ecuda::true_type)
      : base_type(unmanaged_cast(src.get_pointer()), src.number_rows() * src.number_columns(), src.number_depths())
...
public:
    ///
    /// \brief Constructs a cube with dimensions numberRows x numberColumns x numberDepths filled with copies of
    ...
    ///
    __HOST__ cube(const size_type numberRows = 0,
                  const size_type numberColumns = 0,
                  const size_type numberDepths = 0,
                  const value_type& value = value_type(),
                  const Alloc& allocator = Alloc())
      : base_type(pointer(), numberRows * numberColumns, numberDepths)
    ...
    {
        if (number_rows() != src.number_rows() || number_columns() != src.number_columns() ||
            number_depths() != src.number_depths()) {
    ...
    ///
    __HOST__ __DEVICE__ inline size_type number_columns() const ECUDA__NOEXCEPT
    {
        return base_type::number_rows() / numberRows;
    } // TODO: this costs a register in kernel code
    ...
    __HOST__ __DEVICE__ depth_type get_depth(const size_type rowIndex, const size_type columnIndex)
    {
        ...
        ptr +=
          rowIndex * number_columns() * number_depths() + columnIndex * number_depths(); // move pointer to depth start
    ...
    ///
    /// \brief Returns a constant reference to the element at specified row, column, and depth index, with bounds
    /// checking.
    ///
    /// If the row, column, and depth are not within the range of the container, the current kernel will exit and
    /// cudaGetLastError will return cudaErrorUnknown.
    ...
    /// \returns Reference to the requested element.
    ///
    __DEVICE__ inline reference operator()(const size_type rowIndex,
    ...
    /// \returns Reference to the requested element.
    ///
    __DEVICE__ inline const_reference operator()(const size_type rowIndex,
                                                 const size_type columnIndex,
                                                 const size_type depthIndex) const
    {
        return base_type::at(rowIndex * number_columns() + columnIndex, depthIndex);
    }
    ...
    /// \returns view object for the specified row
    ///
    __HOST__ __DEVICE__ inline const_slice_yz_type operator[](const size_type rowIndex) const
    {
        return get_yz(rowIndex);
    }
    ...
    __HOST__ void resize(const size_type newNumberRows,
                         ...
                         const value_type& value = value_type())
    {
        if (number_rows() == newNumberRows && number_columns() == newNumberColumns &&
            number_depths() == newNumberDepths)
        ...
        for (size_type i = 0; i < std::min(newNumberRows, number_rows()); ++i) {
            for (size_type j = 0; j < std::min(newNumberColumns, number_columns()); ++j) {
                ...
                ecuda::copy(inputDepth.begin(),
                            inputDepth.begin() + std::min(newNumberDepths, number_depths()),
                            outputDepth.begin());
            }
        }
...
public:
    template<class P>
    __HOST__ cube_kernel_argument(const cube<T, Alloc, P>& src)
      : base_type(src, ecuda::true_type())
    {
    }

    __HOST__ __DEVICE__ cube_kernel_argument(const cube_kernel_argument& src)
      : base_type(src, ecuda::true_type())
    {
    }
...
#ifdef ECUDA_CPP11_AVAILABLE
    cube_kernel_argument(cube_kernel_argument&& src)
      : base_type(std::move(src))
    {
    }
