...
class padded_ptr; // forward declaration
...
///
struct device_contiguous_iterator_tag : ::std::random_access_iterator_tag
{}; // libc++ requires inheritance from one of the 5 STL iterator categories

///
/// \brief Iterator category denoting device memory that is made of contiguous blocks (but the blocks themselves are
/// non-contiguous).
///
...
public:
    ...
    __HOST__ device_iterator(device_iterator&& src)
    ...
    __HOST__ __DEVICE__ inline device_iterator& operator++()
    {
        ++ptr;
        return *this;
    }
    ...
    __HOST__ __DEVICE__ inline bool operator==(const device_iterator& other) const ECUDA__NOEXCEPT
    {
...
class device_contiguous_iterator
  : public device_iterator<T, typename ecuda::add_pointer<T>::type, device_contiguous_iterator_tag>
{
...
public:
...
#ifdef ECUDA_CPP11_AVAILABLE
    __HOST__ device_contiguous_iterator(device_contiguous_iterator&& src)
    ...
    __HOST__ __DEVICE__ inline device_contiguous_iterator operator+(difference_type x) const
    {
        return device_contiguous_iterator(base_type::ptr + x);
    }
    __HOST__ __DEVICE__ inline device_contiguous_iterator operator-(difference_type x) const
    {
        return device_contiguous_iterator(base_type::ptr - x);
    ...
    __HOST__ __DEVICE__ inline device_contiguous_iterator& operator+=(difference_type x)
    {
        base_type::ptr += x;
        return *this;
    }
    ...
    __HOST__ __DEVICE__ inline difference_type operator-(const device_contiguous_iterator& other)
    {
        return base_type::ptr - other.ptr;
    }

    __HOST__ __DEVICE__ inline bool operator<(const device_contiguous_iterator& other) const ECUDA__NOEXCEPT
    {
        return base_type::ptr < other.ptr;
    }
    __HOST__ __DEVICE__ inline bool operator>(const device_contiguous_iterator& other) const ECUDA__NOEXCEPT
...
class device_contiguous_block_iterator
  : public device_iterator<T, padded_ptr<T, P>, device_contiguous_block_iterator_tag>
{
...
public:
    __HOST__ __DEVICE__ device_contiguous_block_iterator(const pointer& ptr = pointer(),
    ...
    __HOST__ __DEVICE__ inline device_contiguous_block_iterator operator-(difference_type x) const
    {
        return operator+(-x);
    }
    ...
    __HOST__ __DEVICE__ device_contiguous_block_iterator& operator+=(difference_type x)
    {
        ...
        offset += x;
        if (offset > width) {
            base_type::ptr.skip_bytes(base_type::ptr.get_pitch() - width * sizeof(value_type));
            offset -= width;
        }
        if (offset < 0) {
            base_type::ptr.skip_bytes(width * sizeof(value_type) - base_type::ptr.get_pitch());
            offset += width;
        }
        return *this;
    }
    __HOST__ __DEVICE__ inline device_contiguous_block_iterator& operator-=(difference_type x)
    {
        operator+=(-x);
        return *this;
    }
    ...
    __HOST__ __DEVICE__ inline bool operator<(const device_contiguous_block_iterator& other) const ECUDA__NOEXCEPT
    {
        return base_type::ptr < other.ptr;
    }
    __HOST__ __DEVICE__ inline bool operator>(const device_contiguous_block_iterator& other) const ECUDA__NOEXCEPT
    ...
    __HOST__ __DEVICE__ inline contiguous_iterator contiguous_begin() const ECUDA__NOEXCEPT
    {
        return contiguous_iterator(naked_cast<typename ecuda::add_pointer<T>::type>(base_type::ptr.get()));
    }
    __HOST__ __DEVICE__ inline contiguous_iterator contiguous_end() const ECUDA__NOEXCEPT
    {
        return contiguous_iterator(naked_cast<typename ecuda::add_pointer<T>::type>(base_type::ptr.get()) +
                                   (width - offset));
    }
    __HOST__ __DEVICE__ inline std::size_t get_width() const ECUDA__NOEXCEPT { return width; }
    __HOST__ __DEVICE__ inline std::size_t get_offset() const ECUDA__NOEXCEPT { return offset; }
};

template<class Iterator>
class reverse_device_iterator //: public std::iterator<device_iterator_tag,typename Iterator::value_type,typename
                              //: Iterator::difference_type,typename Iterator::pointer>
{
private:
    typedef std::iterator<device_iterator_tag,
...
public:
    ...
    __HOST__ __DEVICE__ inline reverse_device_iterator& operator++()
    {
        --parentIterator;
        return *this;
    }
    ...
    __HOST__ __DEVICE__ inline bool operator==(const reverse_device_iterator& other) const
    {
    ...
    __HOST__ __DEVICE__ inline difference_type operator-(const reverse_device_iterator& other)
    {
        return parentIterator - other.parentIterator;
    }

    __HOST__ __DEVICE__ inline reverse_device_iterator operator+(difference_type x) const
    {
        return reverse_device_iterator(parentIterator - x);
    }
    __HOST__ __DEVICE__ inline reverse_device_iterator operator-(difference_type x) const
    {
        return reverse_device_iterator(parentIterator + x);
    ...
    __HOST__ __DEVICE__ inline reverse_device_iterator& operator+=(difference_type x)
    {
        parentIterator -= x;
        return *this;
    }
...
class iterator_traits<device_iterator<T, PointerType, Category>>
  : private std::iterator_traits<device_iterator<T, PointerType, Category>>
{
private:
...
class iterator_traits<device_contiguous_block_iterator<T, P>>
  : private iterator_traits<device_iterator<T, padded_ptr<T, P>, device_contiguous_block_iterator_tag>>
{
private:
...
class iterator_traits<reverse_device_iterator<Iterator>>
  : private std::iterator_traits<reverse_device_iterator<Iterator>>
{
private:
...
advance(InputIterator& iterator,
        Distance n,
...
template<class InputIterator, typename Distance>
__HOST__ __DEVICE__ inline void
advance(InputIterator& iterator, Distance n)
{
...
#else
    ECUDA_STATIC_ASSERT(isIteratorSomeKindOfContiguous,
...
template<class Iterator>
__HOST__ __DEVICE__ inline typename std::iterator_traits<Iterator>::difference_type
...
template<typename T, typename P>
__HOST__ __DEVICE__ inline typename std::iterator_traits<device_contiguous_block_iterator<T, P>>::difference_type
