...
__HOST__ __DEVICE__ inline ForwardIterator
...
template<class ForwardIterator>
__HOST__ __DEVICE__ inline ForwardIterator
max_element(ForwardIterator first, ForwardIterator last, ecuda::true_type)
{
// is an iterator to device memory
    ...
    const bool isIteratorContiguous =
      ecuda::is_same<typename ecuda::iterator_traits<ForwardIterator>::is_contiguous, ecuda::true_type>::value;
    ECUDA_STATIC_ASSERT(isIteratorContiguous,
...
template<class ForwardIterator>
__HOST__ __DEVICE__ inline ForwardIterator
max_element(ForwardIterator first, ForwardIterator last, ecuda::false_type)
{
...
max_element(ForwardIterator first, ForwardIterator last)
{
    impl::max_element(first, last, typename ecuda::iterator_traits<ForwardIterator>::is_device_iterator());
}
