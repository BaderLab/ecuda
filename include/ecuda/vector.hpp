...
class vector_kernel_argument; // forward declaration
...
protected:
    ...
    ///
    template<class Q>
    __HOST__ __DEVICE__ vector(const vector<T, Alloc, Q>& src, ecuda::true_type)
      : base_type(unmanaged_cast(src.get_pointer()))
      , n(src.n)
...
public:
    ...
    ///
    __HOST__ explicit vector(const allocator_type& allocator = allocator_type())
      : base_type()
      , n(0)
      , allocator(allocator)
    {
    ...
    /// \param allocator allocator to use for all memory allocations of this container
    ///
    __HOST__ explicit vector(size_type n, const value_type& value, const allocator_type& allocator = allocator_type())
      : base_type(shared_ptr<T>(allocator.allocate(n)), n)
      , n(n)
      , allocator(allocator)
    {
    ...
    /// \param n the size of the container
    ///
    __HOST__ explicit vector(size_type n)
      : base_type(shared_ptr<T>(Alloc().allocate(n)))
      , n(n)
    {
    ...
    ///
    __HOST__ __DEVICE__ vector(vector&& src)
      : base_type(src)
      , n(std::move(src.n))
    ...
    /// \param allocator allocator to use for all memory allocations of this container
    ///
    __HOST__ __DEVICE__ vector(vector&& src, const allocator_type& allocator)
      : base_type(src)
      , n(std::move(src.n))
      , allocator(allocator)
    {
    ...
    /// \param allocator allocator to use for all memory allocations of this container
    ///
    __HOST__ vector(std::initializer_list<value_type> il, const allocator_type& allocator = allocator_type())
      : base_type(shared_ptr<T>(allocator.allocate(il.size())))
      , n(il.size())
      , allocator(allocator)
    {
    ...
    ///
    __HOST__ __DEVICE__ inline iterator end() ECUDA__NOEXCEPT { return base_type::begin() + size(); }
    ...
    /// \returns Iterator to the element following the last element.
    ///
    __HOST__ __DEVICE__ inline const_iterator end() const ECUDA__NOEXCEPT { return base_type::begin() + size(); }
    ...
    ///
    __HOST__ __DEVICE__ inline size_type size() const ECUDA__NOEXCEPT { return n; }
    ...
    ///
    __HOST__ __DEVICE__ inline size_type capacity() const ECUDA__NOEXCEPT { return base_type::size(); }
    ...
    ///
    __HOST__ __DEVICE__ inline bool empty() const ECUDA__NOEXCEPT { return !n; }
    ...
    { /*if( n )*/
        --n;
    } // NOTE: if called from device the host instance doesn't change
    ...
    {
        std::vector<value_type, host_allocator<value_type>> v(::ecuda::distance(position, end()) +
    ...
    {
        std::vector<value_type, host_allocator<value_type>> v(::ecuda::distance(position, end()) + count,
                                                              value); // allocate staging memory
    ...
    {
        const std::vector<value_type, host_allocator<value_type>> v(
          first, last);                                          // allocate staging memory and put new content there
    ...
    ///
    /// \param other container to compare contents with
    /// \returns true if the contents of this vector are lexicographically greater than the other vector, false
    /// otherwise
    ///
    __HOST__ __DEVICE__ inline bool operator>(const vector& other) const
    {
    ...
    ///
    /// \param other container to compare contents with
    /// \returns true if the contents of this vector are lexicographically less than or equal to the other vector, false
    /// otherwise
    ///
    __HOST__ __DEVICE__ inline bool operator<=(const vector& other) const { return !operator>(other); }
    ...
    ///
    /// \param other container to compare contents with
    /// \returns true if the contents of this vector are lexicographically greater than or equal to the other vector,
    /// false otherwise
    ///
    __HOST__ __DEVICE__ inline bool operator>=(const vector& other) const { return !operator<(other); }
};
...
__HOST__ void
vector<T, Alloc, P>::growMemory(size_type minimum)
{
...
public:
    template<class P>
    __HOST__ vector_kernel_argument(const vector<T, Alloc, P>& src)
      : base_type(src, ecuda::true_type())
    {
    }
    __HOST__ __DEVICE__ vector_kernel_argument(const vector_kernel_argument& src)
      : base_type(src, ecuda::true_type())
    {
    }
    template<class P>
...
#ifdef ECUDA_CPP11_AVAILABLE
    vector_kernel_argument(vector_kernel_argument&& src)
