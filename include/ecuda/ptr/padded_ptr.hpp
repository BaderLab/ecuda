...
private:
    ...
    template<typename U>
    __HOST__ __DEVICE__ typename char_pointer<U*>::type char_cast(U* ptr) const
    {
        return reinterpret_cast<char*>(ptr);
    }
    template<typename U>
    __HOST__ __DEVICE__ typename char_pointer<const U*>::type char_cast(const U* ptr) const
    {
...
public:
    ...
    __HOST__ __DEVICE__ padded_ptr(padded_ptr&& src)
    ...
    {
        return padded_ptr(*this).operator+=(i).operator*();
    }
    ...
    ///
    __HOST__ __DEVICE__ explicit operator bool() const ECUDA__NOEXCEPT
    {
        return naked_cast<typename ecuda::add_pointer<const element_type>::type>(ptr) != NULL;
    }
#else
    ///
    /// \brief Checks if this stores a non-null pointer.
    ...
    /// \return true if *this stores a pointer, false otherwise.
    ///
    __HOST__ __DEVICE__ operator bool() const ECUDA__NOEXCEPT
    {
        return naked_cast<typename ecuda::add_pointer<const element_type>::type>(ptr) != NULL;
    }
#endif
    ...
    __HOST__ __DEVICE__ inline padded_ptr operator++(int)
    {
        padded_ptr tmp(*this);
        ++(*this);
        return tmp;
