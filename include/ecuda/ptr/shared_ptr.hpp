...
struct sp_counter_impl_p : sp_counter_base
{
    ...
    {
        if (p) ecuda::default_device_delete<void>()(p);
    } // cudaFree( p );
};
...
{
    Deleter deleter;
    sp_counter_impl_pd(Deleter deleter)
      : sp_counter_base()
      , deleter(deleter)
    {
    }
...
private:
    void* current_ptr; //!< the stored pointer to the object itself is anonymous (sp_counter_base relies on a void*
                       //!< pointer for dispose)
    detail::sp_counter_base*
      counter; //!< pointer to shared counting/disposal structure (is NULL iff current_ptr is NULL)
...
public:
    ...
    ///
    __HOST__ __DEVICE__ ECUDA__CONSTEXPR shared_ptr() ECUDA__NOEXCEPT
      : current_ptr(NULL)
      , counter(NULL)
    {
    }
    ...
    ///
    template<typename U>
    __HOST__ __DEVICE__ explicit shared_ptr(U* ptr)
      : current_ptr(detail::void_cast<T*>()(ptr))
      , counter(NULL)
    {
    ...
    ///
    template<typename U>
    __HOST__ __DEVICE__ shared_ptr(const shared_ptr<U>& src, T* ptr) ECUDA__NOEXCEPT
      : current_ptr(ptr)
      , counter(src.counter)
    {
#ifndef __CUDA_ARCH__
    ...
    /// \param src another smart pointer to share ownership to or acquire the ownership from
    ///
    __HOST__ __DEVICE__ shared_ptr(const shared_ptr& src) ECUDA__NOEXCEPT
      : current_ptr(src.current_ptr)
      , counter(src.counter)
    {
#ifndef __CUDA_ARCH__
    ...
    ///
    template<typename U>
    __HOST__ __DEVICE__ shared_ptr(const shared_ptr<U>& src) ECUDA__NOEXCEPT
      : current_ptr(src.current_ptr)
      , counter(src.counter)
    {
#ifndef __CUDA_ARCH__
    ...
    /// \param src another smart pointer to share ownership to or acquire the ownership from
    ///
    __HOST__ __DEVICE__ shared_ptr(shared_ptr&& src) ECUDA__NOEXCEPT
      : current_ptr(std::move(src.current_ptr))
      , counter(std::move(src.counter))
    {
    ...
    ///
    template<typename U>
    __HOST__ __DEVICE__ shared_ptr(shared_ptr<U>&& src) ECUDA__NOEXCEPT
      : current_ptr(std::move(src.current_ptr))
      , counter(std::move(src.counter))
    {
        // src.current_ptr = NULL;
    ...
    ///
    template<typename U, class Deleter>
    __HOST__ __DEVICE__ shared_ptr(ecuda::unique_ptr<U, Deleter>&& src)
      : current_ptr(detail::void_cast<T*>()(src.release()))
    {
    ...
    ///
    template<typename U>
    __HOST__ __DEVICE__ inline shared_ptr& operator=(const shared_ptr<U>& src) ECUDA__NOEXCEPT
    {
        shared_ptr(src).swap(*this);
        return *this;
    }
    ...
    /// \param src another smart pointer to share ownership to or acquire the ownership from
    ///
    __HOST__ __DEVICE__ inline shared_ptr& operator=(shared_ptr&& src) ECUDA__NOEXCEPT
    {
        shared_ptr(move(src)).swap(*this);
        return *this;
    }
    ...
    ///
    template<typename U>
    __HOST__ __DEVICE__ inline shared_ptr& operator=(shared_ptr<U>&& src) ECUDA__NOEXCEPT
    {
        shared_ptr(move(src)).swap(*this);
        return *this;
    }
    ...
    ///
    template<typename U, class Deleter>
    __HOST__ __DEVICE__ inline shared_ptr& operator=(unique_ptr<U, Deleter>&& src) ECUDA__NOEXCEPT
    {
        shared_ptr(move(src)).swap(*this);
        return *this;
    }
    ...
    ///
    template<typename U>
    __HOST__ __DEVICE__ inline void reset(U* ptr)
    {
    ...
    ///
    template<typename U, class Deleter>
    __HOST__ __DEVICE__ inline void reset(U* ptr, Deleter d)
    {
    ...
    {
        return *reinterpret_cast<T*>(current_ptr);
    }
    ...
    ///
    __HOST__ __DEVICE__ inline std::size_t use_count() const ECUDA__NOEXCEPT
    {
        return counter ? counter->owner_count : 0;
    }
    ...
    ///
    template<typename U>
    __HOST__ __DEVICE__ inline bool owner_before(const shared_ptr<U>& other) const
    {
        return counter < other.counter;
    }

    template<typename T2>
    __HOST__ __DEVICE__ bool operator==(const shared_ptr<T2>& other) const ECUDA__NOEXCEPT
    {
