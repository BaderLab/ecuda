...
public:
    ...
    typedef device_sequence<value_type, striding_ptr<value_type, typename ecuda::add_pointer<value_type>::type>>
      column_type;
    typedef device_sequence<const value_type,
                            striding_ptr<const value_type, typename ecuda::add_pointer<const value_type>::type>>
      const_column_type;
...
public:
    __HOST__ __DEVICE__ device_contiguous_matrix(pointer ptr = pointer(), size_type rows = 0, size_type columns = 0)
      : base_type(ptr, rows * columns)
      , rows(rows)
    {
    }

    __HOST__ __DEVICE__ device_contiguous_matrix(const device_contiguous_matrix& src)
      : base_type(src)
      , rows(src.rows)
    {
    }
    ...
    __HOST__ __DEVICE__ device_contiguous_matrix(const device_contiguous_matrix<U, Q>& src)
      : base_type(src)
      , rows(src.rows)
    {
    }
    ...
    __HOST__ device_contiguous_matrix(device_contiguous_matrix&& src)
    ...
    __HOST__ __DEVICE__ row_type get_row(const size_type row)
    {
        return row_type(unmanaged_cast(base_type::get_pointer()) + (row * number_columns()), number_columns());
    }
    __HOST__ __DEVICE__ const_row_type get_row(const size_type row) const
    {
    ...
    {
        return column_type(striding_ptr<value_type, typename make_unmanaged<pointer>::type>(
    ...
    {
        return const_column_type(striding_ptr<const value_type, typename make_unmanaged_const<pointer>::type>(
                                   unmanaged_cast(base_type::get_pointer()) + column, // move to top of column
                                   number_columns()                                   // stride by number of columns
                                   ),
                                 number_rows());
    ...
    {
        return *naked_cast<typename ecuda::add_pointer<value_type>::type>(base_type::get_pointer() +
                                                                          (row * number_columns()) + column);
    }

    __DEVICE__ inline const_reference operator()(const size_type row, const size_type column) const
    {
        return *naked_cast<typename ecuda::add_pointer<const value_type>::type>(base_type::get_pointer() +
                                                                                (row * number_columns()) + column);
    }
    ...
    __DEVICE__ inline reference at(const size_type rowIndex, const size_type columnIndex)
    {
        if (rowIndex >= number_rows() || columnIndex >= number_columns()) {
            ...
            throw std::out_of_range(ECUDA_EXCEPTION_MSG(
              "ecuda::model::device_contiguous_matrix::at() row and/or column index parameter is out of range"));
#else
