...
public:
    __HOST__ __DEVICE__ device_matrix(pointer ptr = pointer(), size_type rows = 0, size_type columns = 0)
      : base_type(ptr, rows * columns)
      , rows(rows)
    {
    }

    __HOST__ __DEVICE__ device_matrix(const device_matrix& src)
      : base_type(src)
      , rows(src.rows)
    {
    }
    ...
    template<typename U, typename Q>
    __HOST__ __DEVICE__ device_matrix(const device_matrix<U, Q>& src)
      : base_type(src)
      , rows(src.rows)
    {
    }
    ...
    __HOST__ device_matrix(device_matrix&& src)
      : base_type(src)
    ...
    __DEVICE__ inline reference at(const size_type rowIndex, const size_type columnIndex)
    {
        if (rowIndex >= number_rows() || columnIndex >= number_columns()) {
            ...
            throw std::out_of_range(ECUDA_EXCEPTION_MSG(
              "ecuda::model::device_matrix::at() row and/or column index parameter is out of range"));
#else
