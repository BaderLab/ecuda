...
public:
    ...
    template<typename U, std::size_t M, typename Q>
    friend class device_fixed_sequence;
...
public:
    __HOST__ __DEVICE__ device_fixed_sequence(pointer ptr = pointer())
      : ptr(ptr)
    {
    }

    __HOST__ __DEVICE__ device_fixed_sequence(const device_fixed_sequence& src)
      : ptr(src.ptr)
    {
    }

    template<typename U, typename Q>
    __HOST__ __DEVICE__ device_fixed_sequence(const device_fixed_sequence<U, N, Q>& src)
      : ptr(src.ptr)
    {
    }
    ...
    __DEVICE__ inline reference at(const size_type x)
    {
        if (x >= size()) {
            ...
            throw std::out_of_range(
              ECUDA_EXCEPTION_MSG("ecuda::model::device_fixed_sequence::at() index parameter is out of range"));
#else
