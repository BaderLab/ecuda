...
public:
    ...
    typedef device_contiguous_block_iterator<value_type, typename make_unmanaged<P>::type>
      iterator; // this iterator's 2nd template param is also padded_ptr<T,P>
    ...
    typedef device_sequence<value_type, striding_padded_ptr<value_type, typename ecuda::add_pointer<value_type>::type>>
      column_type;
    typedef device_sequence<const value_type,
                            striding_padded_ptr<const value_type, typename ecuda::add_pointer<const value_type>::type>>
      const_column_type;

public:
    __HOST__ __DEVICE__ device_contiguous_row_matrix(pointer ptr = pointer(), size_type rows = 0, size_type columns = 0)
      : base_type(ptr, rows, columns)
    {
    }

    __HOST__ __DEVICE__ device_contiguous_row_matrix(const device_contiguous_row_matrix& src)
      : base_type(src)
    {
    }

    template<typename U, class Q>
    __HOST__ __DEVICE__ device_contiguous_row_matrix(const device_contiguous_row_matrix<U, Q>& src)
      : base_type(src)
    {
    }
    ...
    __HOST__ device_contiguous_row_matrix(device_contiguous_row_matrix&& src)
      : base_type(src)
    {
    }
    ...
    __HOST__ __DEVICE__ inline iterator begin() ECUDA__NOEXCEPT
    {
...
// return const_iterator( unmanaged_cast(base_type::get_pointer())+base_type::size() ); }
    ...
    __HOST__ __DEVICE__ inline row_type get_row(const size_type row)
    {
        ...
        return row_type(naked_cast<typename ecuda::add_pointer<value_type>::type>(up),
                        base_type::number_columns()); // provide naked pointer since row is contiguous
    }
    ...
    __HOST__ __DEVICE__ inline const_row_type get_row(const size_type row) const
    {
        ...
        return const_row_type(naked_cast<typename ecuda::add_pointer<const value_type>::type>(up),
                              base_type::number_columns()); // provide naked pointer since row is contiguous
    }
    ...
    __HOST__ __DEVICE__ inline column_type get_column(const size_type column)
    {
        ...
        return column_type(
          striding_padded_ptr<value_type, naked_pointer_type>(np, base_type::get_pointer().get_pitch()),
          base_type::number_rows());
    }
    ...
    __HOST__ __DEVICE__ inline const_column_type get_column(const size_type column) const
    {
        ...
        return const_column_type(
          striding_padded_ptr<const value_type, naked_pointer_type>(np, base_type::get_pointer().get_pitch()),
          base_type::number_rows());
    }
...
#ifndef __CUDACC__
            throw std::out_of_range(ECUDA_EXCEPTION_MSG(
              "ecuda::model::device_contiguous_row_matrix::at() row and/or column index parameter is out of range"));
#else
